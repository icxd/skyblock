 ...
 * contradict.
 *
 * <p>Special rooms: at least 2 puzzles on every floor and up to 5 on F7, one miniboss (yellow)
 * room per floor, one trap room from F3 up. The exact puzzle range per floor isn't documented; the
 * numbers below are a best guess in between.
 *
 * <p>{@code specialColumn}: on F4-F6 Odin's map assumes the last column holds the puzzles, trap
 * and miniboss room ({@code SpecialColumn.kt}), i.e. those floors are a 5-wide dungeon with a
 * column of special rooms bolted on.
 */
@Getter
public enum DungeonFloor {
    ...
    FLOOR_4("Floor 4", false, 4, 6, 5, 3, 3, 1, true),
    FLOOR_5("Floor 5", false, 5, 6, 6, 3, 4, 1, true),
    FLOOR_6("Floor 6", false, 6, 6, 6, 3, 4, 1, true),
    FLOOR_7("Floor 7", false, 7, 6, 6, 3, 5, 1, false),
    ...
    MASTER_FLOOR_4("Master Floor 4", true, 4, 6, 5, 3, 3, 1, true),
    MASTER_FLOOR_5("Master Floor 5", true, 5, 6, 6, 3, 4, 1, true),
    MASTER_FLOOR_6("Master Floor 6", true, 6, 6, 6, 3, 4, 1, true),
    MASTER_FLOOR_7("Master Floor 7", true, 7, 6, 6, 3, 5, 1, false);
